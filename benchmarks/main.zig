...
pub fn main() !void {
    ...
    for (results.items) |result| {
        const target_ops_per_sec: f64 = switch (std.mem.indexOf(u8, result.name, "任务调度")) {
            null => switch (std.mem.indexOf(u8, result.name, "Future轮询")) {
                null => 1_000_000, // 默认目标：100万ops/sec
                else => 10_000_000, // Future轮询目标：1000万ops/sec
            },
            else => 5_000_000, // 任务调度目标：500万ops/sec
        };
