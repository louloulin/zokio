...
pub const PlatformCapabilities = struct {
    ...
    /// 编译时检测平台是否支持
    pub const is_supported = comptime blk: {
        // 至少需要一个I/O后端
    ...
    /// 编译时选择首选I/O后端
    pub const preferred_io_backend = comptime blk: {
    ...
    /// 编译时检测是否支持多线程
    pub const threading_available = comptime builtin.os.tag != .wasi;
    
    /// 编译时确定最优工作线程数
    pub const optimal_worker_count = comptime blk: {
        if (!threading_available) break :blk 1;
        
        // 尝试获取CPU核心数，失败则使用默认值
        const cpu_count = std.Thread.getCpuCount() catch 4;
        break :blk @min(cpu_count, 64); // 最多64个工作线程
    };
};
