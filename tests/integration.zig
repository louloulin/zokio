...
test "Zokio运行时集成测试" {
    ...
    // 测试编译时信息
    try testing.expect(config.ZokioRuntime(config).COMPILE_TIME_INFO.worker_threads > 0);
    try testing.expect(config.ZokioRuntime(config).PERFORMANCE_CHARACTERISTICS.theoretical_max_tasks_per_second > 0);
}
